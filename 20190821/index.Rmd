...
(1) Go to this address <https://rstudio.cloud/project/446255>. This hosted RStudio instance contains materials for today's workshop.
...
(2) Ask me any question at any time by going to [slido.com](https://www.sli.do) and enter event code #8464 (or use [this link](https://app.sli.do/event/7ev3yywn)). I'll try to check these questions periodically (upvote questions if you'd like them answered!)
...
Every plotly.js figure follows a JSON specification. Most importantly, every figure is made up of one or more traces (a trace is a mapping from data to visual).
...
Let's pull the relevant data into a `tibble()` and map columns to the appropriate plotly.js attributes (e.g., `x`, `y`, `name`).
...
The remainder of the workshop is about [linking views *server-side* with Shiny](https://plotly-r.com/linking-views-with-shiny.html)...let's talk about the difference
...
# Easy to share, scale, and maintain
...
# But, way more powerful and productive! 
...
.footnote[
Useful pattern to follow for any high-dimensional datavis problem
]
...
Add a title to the sub-category pie chart showing what category is currently selected.

```{r, echo=FALSE, eval = TRUE}
countdown(minutes = 2, seconds = 0, bottom = 0, color_background = "black")
```
