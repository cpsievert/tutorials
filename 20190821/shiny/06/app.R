...
server <- function(input, output, session) {
  ...
  output$p <- renderPlotly({
    outcome_pca %>%
      ...
      add_markers(
        ...
        marker = list(color = rep("black", nrow(outcome_pca)))
      )
  })
  ...
  observeEvent(event_data("plotly_click"), {
    ...
    idx <- which(outcome_pca$drug %in% click) - 1
    update <- setNames(list(toRGB(names(click))), paste0("marker.color[", idx, "]"))
    plotlyProxy("p", session) %>%
      plotlyProxyInvoke("restyle", update, list(0))
  })
